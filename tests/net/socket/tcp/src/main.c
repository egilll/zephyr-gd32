...
static void test_tcp_info_common(int af)
{
	struct zsock_tcp_info info;
	net_socklen_t optlen;
	uint8_t tx_large[2048] = { 0 };
	char tx_small[] = TEST_STR_SMALL;
	size_t send_len;
	int outq;
	int ret;
	/* server fd := 0, client fd := 1, accept fd := 2 */
	...
	optlen = sizeof(info);
	ret = zsock_getsockopt(fd[CLIENT], NET_IPPROTO_TCP, ZSOCK_TCP_INFO, &info, &optlen);
	zassert_ok(ret, "getsockopt(TCP_INFO) failed (%d)", errno);
	zassert_equal(optlen, sizeof(info), "wrong TCP_INFO length");
	zassert_equal(info.tcpi_state, ZSOCK_TCP_ESTABLISHED, "unexpected TCP state");
	zassert_equal(info.tcpi_ca_state, ZSOCK_TCP_CA_OPEN, "unexpected CA state");
	zassert_equal(info.tcpi_retransmits, 0U, "unexpected retransmit count");
	zassert_equal(info.tcpi_dupacks, 0U, "unexpected dupack count");
	zassert_true(info.tcpi_rto > 0U, "expected tcpi_rto > 0");
	zassert_true(info.tcpi_snd_mss > 0U, "expected tcpi_snd_mss > 0");
	zassert_true(info.tcpi_snd_wnd > 0U, "expected tcpi_snd_wnd > 0");
	zassert_true(info.tcpi_rcv_space > 0U, "expected tcpi_rcv_space > 0");
	zassert_equal(info.tcpi_snd_una, info.tcpi_snd_nxt, "expected empty unacked range");
	zassert_equal(info.tcpi_snd_nxt, info.tcpi_snd_max, "expected empty queued range");
	zassert_equal(info.tcpi_notsent_bytes, 0U, "expected no queued notsent bytes");

	outq = -1;
	zassert_ok(zsock_ioctl(fd[CLIENT], SIOCOUTQ, &outq));
	zassert_equal(outq, 0, "expected empty outq");

	zassert_equal(loopback_set_packet_drop_ratio(1.0f), 0,
		      "cannot set packet loss");

	ret = zsock_send(fd[CLIENT], tx_small, sizeof(tx_small), ZSOCK_MSG_DONTWAIT);
	zassert_equal(ret, sizeof(tx_small), "unexpected small send %d", ret);

	outq = -1;
	zassert_ok(zsock_ioctl(fd[CLIENT], SIOCOUTQ, &outq));
	zassert_equal(outq, sizeof(tx_small), "wrong queued send bytes");
	...
	zassert_true(info.tcpi_snd_mss + 1U < sizeof(tx_large),
		     "test buffer too small for MSS %u", info.tcpi_snd_mss);
	send_len = info.tcpi_snd_mss + 1U;

	zassert_equal(loopback_set_packet_drop_ratio(1.0f), 0,
		      "cannot set packet loss");

	ret = zsock_send(fd[CLIENT], tx_large, send_len, ZSOCK_MSG_DONTWAIT);
	zassert_equal(ret, send_len, "unexpected queued send %d", ret);

	outq = -1;
	zassert_ok(zsock_ioctl(fd[CLIENT], SIOCOUTQ, &outq));
	zassert_equal(outq, send_len, "wrong queued send bytes");

	optlen = sizeof(info);
	ret = zsock_getsockopt(fd[CLIENT], NET_IPPROTO_TCP, ZSOCK_TCP_INFO, &info, &optlen);
	zassert_ok(ret, "getsockopt(TCP_INFO) failed (%d)", errno);
	zassert_equal(info.tcpi_state, ZSOCK_TCP_ESTABLISHED, "unexpected TCP state");
	zassert_equal(info.tcpi_notsent_bytes, 1U, "wrong TCP_INFO notsent bytes");
	zassert_equal(info.tcpi_snd_nxt - info.tcpi_snd_una, info.tcpi_snd_mss,
		      "wrong unacked sequence range");
	zassert_equal(info.tcpi_snd_max - info.tcpi_snd_nxt, 1U,
		      "wrong queued notsent sequence range");
...
ZTEST(net_socket_tcp, test_tcp_info_v4)
{
	test_tcp_info_common(NET_AF_INET);
}

ZTEST(net_socket_tcp, test_tcp_info_v6)
{
	test_tcp_info_common(NET_AF_INET6);
}

static void test_tcp_info_listen_common(int af)
{
	struct zsock_tcp_info info;
	net_socklen_t optlen = sizeof(info);
	int fd = -1;
	int ret;
	...
	if (af == NET_AF_INET) {
		...
		prepare_sock_tcp_v6(MY_IPV6_ADDR, SERVER_PORT, &fd, &addr);
		test_bind(fd, (struct net_sockaddr *)&addr, sizeof(addr));
	}

	test_listen(fd);

	ret = zsock_getsockopt(fd, NET_IPPROTO_TCP, ZSOCK_TCP_INFO, &info, &optlen);
	zassert_ok(ret, "getsockopt(TCP_INFO) failed (%d)", errno);
	zassert_equal(optlen, sizeof(info), "wrong TCP_INFO length");
	zassert_equal(info.tcpi_state, ZSOCK_TCP_LISTEN, "unexpected TCP state");
	zassert_equal(info.tcpi_snd_una, info.tcpi_snd_nxt, "expected empty unacked range");
	zassert_equal(info.tcpi_snd_nxt, info.tcpi_snd_max, "expected empty queued range");
	zassert_equal(info.tcpi_notsent_bytes, 0U, "expected no queued notsent bytes");
...
ZTEST(net_socket_tcp, test_tcp_info_listen_v4)
{
	test_tcp_info_listen_common(NET_AF_INET);
}

ZTEST(net_socket_tcp, test_tcp_info_listen_v6)
{
	test_tcp_info_listen_common(NET_AF_INET6);
}

ZTEST(net_socket_tcp, test_tcp_listen_ioctls)
{
	struct net_sockaddr_in addr;
	int avail = -1;
	int fd;
	...
	ret = zsock_ioctl(fd, FIONREAD, &avail);
	zassert_equal(ret, -1, "FIONREAD unexpectedly succeeded");
	zassert_equal(errno, EINVAL, "unexpected errno %d", errno);

	ret = zsock_ioctl(fd, SIOCOUTQ, &avail);
	zassert_equal(ret, -1, "SIOCOUTQ unexpectedly succeeded");
	zassert_equal(errno, EINVAL, "unexpected errno %d", errno);
...
ZTEST(net_socket_tcp, test_tcp_info_invalid_optlen)
{
	struct zsock_tcp_info info;
	net_socklen_t optlen = sizeof(info) - 1;
	struct net_sockaddr_in addr = { 0 };
	int fd;
	int ret;

	prepare_sock_tcp_v4(MY_IPV4_ADDR, ANY_PORT, &fd, &addr);

	ret = zsock_getsockopt(fd, NET_IPPROTO_TCP, ZSOCK_TCP_INFO, &info, &optlen);
	zassert_equal(ret, -1, "getsockopt(TCP_INFO) unexpectedly succeeded");
	zassert_equal(errno, EINVAL, "unexpected errno %d", errno);
...
ZTEST(net_socket_tcp, test_tcp_info_on_udp_socket)
{
	struct zsock_tcp_info info;
	net_socklen_t optlen = sizeof(info);
	int fd;
