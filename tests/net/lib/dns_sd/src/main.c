...
static int build_expected_srv_record(uint8_t *buf, size_t buf_size,
				     uint16_t instance_offset,
				     uint16_t domain_offset)
{
	...
	zassert_true(hostname_len > 0, "hostname must not be empty");
	zassert_true(hostname_len <= UINT8_MAX, "hostname too long");

	rdlength = sizeof(struct dns_srv_rdata) + DNS_LABEL_LEN_SIZE +
		   hostname_len + DNS_POINTER_SIZE;

	zassert_true(buf_size >= sizeof(uint16_t) + sizeof(struct dns_rr) + rdlength,
		     "buffer too small for expected SRV record");
...
static int build_expected_ptr_query_response(uint8_t *buf, size_t buf_size)
{
	static const uint8_t prefix[] = {
		...
		0x06, 0x70, 0x61, 0x74, 0x68, 0x3d, 0x2f,
	};
	size_t offset = 0;
	uint16_t host_offset;
	int srv_size;

	zassert_true(buf_size >= sizeof(prefix) + 16,
		     "buffer too small for expected DNS-SD response");

	memcpy(&buf[offset], prefix, sizeof(prefix));
	offset += sizeof(prefix);

	host_offset = offset + 18;
	srv_size = build_expected_srv_record(&buf[offset], buf_size - offset,
					     0x28, 0x17);
	offset += srv_size;
...
ZTEST(dns_sd, test_dns_sd_handle_ptr_query)
{
	...
	int expected_int = build_expected_ptr_query_response(expected_rsp, sizeof(expected_rsp));
