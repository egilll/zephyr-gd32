...
enumeration. Questions for absent types at an owned service-instance name
receive a restricted ``NSEC`` answer. Browse replies include the applicable
``SRV``, ``TXT``, ``A``, and ``AAAA`` additional records. When the target has
addresses from only one IP family, browse and direct ``SRV`` replies include a
restricted ``NSEC`` additional record for the absent family. Known answers in
multicast questions suppress matching host, browse, and service-instance
answers.
