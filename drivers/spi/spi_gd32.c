...
{
	return SPI_OP_MODE_GET(operation) == SPI_OP_MODE_PERIPHERAL;
}
	...
	if (data->xfer_mode == SPI_GD32_XFER_DMA) {
		for (size_t i = 0; i < NUM_OF_DIRECTION; i++) {
			if (cfg->dma[i].dev != NULL) {
				int ret = dma_stop(cfg->dma[i].dev, cfg->dma[i].channel);

				if (ret != 0) {
					LOG_ERR("%s failed to stop %s DMA channel: %d", dev->name,
						i == TX ? "TX" : "RX", ret);
				}
			}
		}
				  ...
				  const struct spi_buf_set *rx_bufs, uint8_t dfs)
{
	...
	/* GD32F4 full-duplex 16-bit DMA can retire with RBNE and RXORERR still set. */
	if (dfs != 1U || spi_gd32_is_slave(data) || !spi_gd32_dma_enabled(dev)) {
		return false;
	}
...
static bool spi_gd32_dma_channel_complete(const struct spi_gd32_dma_config *dma, bool callback_done)
{
	...
	return status.pending_length == 0U && !status.busy;
}
			      ...
			      size_t chunk_len)
{
	...
	*dma_cfg = (struct dma_config){
		.source_burst_length = data->dfs,
		.dest_burst_length = data->dfs,
		.user_data = (void *)dev,
	...
	*block_cfg = (struct dma_block_config){
	...
	if (ret != 0) {
		LOG_ERR("%s failed to configure %s DMA channel: %d", dev->name,
			dir == TX ? "TX" : "RX", ret);
		return ret;
	}
...
static size_t spi_gd32_dma_chunk_len(struct spi_context *ctx)
{
	return MIN(spi_context_max_continuous_chunk(ctx), (size_t)UINT16_MAX);
}

static int spi_gd32_dma_prepare_chunk(const struct device *dev)
{
	struct spi_gd32_data *data = dev->data;
	size_t chunk_len = spi_gd32_dma_chunk_len(&data->ctx);
	int ret;
