...
static uint32_t i2c_gd32_sync_timeout_ms_xfer(const struct device *dev,
					      const struct i2c_msg *msgs, uint8_t num_msgs)
{
	...
	if ((msgs[0].flags & I2C_MSG_ADDR_10_BITS) != 0U) {
...
static int i2c_gd32_segment_init(const struct device *dev, uint8_t start_idx, enum i2c_gd32_seg_end *seg_end)
{
	...
	while (idx < data->num_msgs) {
		if ((UINT32_MAX - data->xfer_len) < msgs[idx].len) {
			return -EOVERFLOW;
		}
...
static void i2c_gd32_segment_complete(const struct device *dev, enum i2c_gd32_seg_end seg_end)
{
	...
	if (i2c_gd32_segment_init(dev, data->seg_next, &next_end) != 0) {
...
#ifdef CONFIG_I2C_GD32_DMA
static void i2c_gd32_handle_addsend_dma(const struct device *dev)
{
	struct i2c_gd32_data *data = dev->data;
	const struct i2c_gd32_config *cfg = dev->config;
	...
	/* DMAON must be set only after ADDSEND is cleared (GD32 RM 20.3.9). */
	...
	/* A 10-bit read addresses as write first, then repeats START with read. */
	if (data->is_restart) {
		data->is_restart = false;
		data->addr_is_read = true;
		data->waiting_for_start = true;
	...
	if (seg_end == I2C_GD32_SEG_END_STOP && data->dma_is_read && data->xfer_len == 1U) {
		I2C_CTL0(cfg->reg) |= I2C_CTL0_STOP;
	}
...
#ifdef CONFIG_I2C_GD32_DMA
		if (data->dma_active) {
			i2c_gd32_handle_addsend_dma(dev);
		} else
#endif
		{
				  ...
				  int status)
{
	...
	if (!data->dma_active || data->dma_done || dma_dev != data->dma_dev ||
	    channel != data->dma_channel) {
		return;
	}
...
static int i2c_gd32_xfer_end(const struct device *dev)
{
	...
	if (data->errs != 0U) {
		if ((data->errs & I2C_GD32_ERR_TIMEOUT) != 0U) {
			return -ETIMEDOUT;
		}
		if ((data->errs & I2C_GD32_ERR_BUSY) != 0U) {
			return -EBUSY;
		}
		if ((data->errs & I2C_GD32_ERR_LARB) != 0U) {
			return -EAGAIN;
		}
		if ((data->errs & I2C_GD32_ERR_AERR) != 0U) {
			return -ENXIO;
		}
		return -EIO;
	}
			     ...
			     uint16_t addr)
{
	...
	addr_10_bits = (msgs[0].flags & I2C_MSG_ADDR_10_BITS) != 0U;
	if (addr > (addr_10_bits ? 0x3ffU : 0x7fU)) {
		return -EINVAL;
	}
	...
	for (uint8_t i = 0U; i < num_msgs; i++) {
		...
		if ((msgs[i].len == 0U) && (num_msgs != 1U)) {
			return -EINVAL;
		}
		if (((msgs[i].flags & I2C_MSG_ADDR_10_BITS) != 0U) != addr_10_bits) {
			return -EINVAL;
		}
		if (i > 0U && (msgs[i].flags & I2C_MSG_RESTART) != 0U) {
			segment_len = 0U;
		}
		if ((UINT32_MAX - segment_len) < msgs[i].len) {
			return -EOVERFLOW;
		}
		segment_len += msgs[i].len;
		if (i < (num_msgs - 1U)) {
	...
	data->addr_10_bits = addr_10_bits;
	if (addr_10_bits) {
		/* Store the seven-bit form; the ISR appends R/W when writing I2C_DATA. */
		data->addr1 = 0x78U | ((addr >> 8U) & 0x3U);
	...
	err = i2c_gd32_segment_init(dev, 0U, &seg_end);
	if (err != 0) {
		goto out;
	}
...
static int i2c_gd32_init(const struct device *dev)
{
	...
	err = clock_control_on(GD32_CLOCK_CONTROLLER, (clock_control_subsys_t)&cfg->clkid);
	if (err != 0) {
		return err;
	}
