				   ...
				   const struct rtc_time *timeptr)
{
	...
	if (mask == 0U) {
		...
		(void)gd32_rtc_alarm_disable(id);
...
static int gd32_rtc_wakeup_config_1hz(void)
{
#if defined(RTC_CTL_WTEN) && defined(RTC_WUT) && defined(WAKEUP_CKSPRE)
	...
	gd32_rtc_wp_unlock();
	RTC_CTL = (RTC_CTL & ~RTC_CTL_WTCS) | (uint32_t)WAKEUP_CKSPRE | RTC_CTL_WTIE;
	RTC_WUT = 0U;
	RTC_CTL |= RTC_CTL_WTEN;
	gd32_rtc_wp_lock();
					...
					void *user_data)
{
	...
	if (callback != NULL) {
		ret = gd32_rtc_wakeup_config_1hz();
	} else {
